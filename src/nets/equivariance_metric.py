...
def make_group_elements(r2_act, thetas):
    ...
    try:
        # If this works, G is SO(2) (continuous rotations)
        elems = [G.element(float(t)) for t in thetas]
        return elems
    except TypeError:
        pass

    # Otherwise use O(2): (flip, angle)
    # flip=0 gives the rotation subgroup, flip=1 adds reflections.
    for flip in (0, 1):
...
def check_equivariance_batch_r2(
    ...
    if isinstance(y_ref, nn.GeometricTensor):
        ...
        y_ref_geo = nn.GeometricTensor(y_ref, output_type)
