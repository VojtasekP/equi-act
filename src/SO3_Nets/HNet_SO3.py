...
class HNet(torch.nn.Module):
    def __init__(self,
        ...
        self.r3_act = gspaces.rot3dOnR3(-1, maximum_frequency=max_rot_order)
    ...
    def make_gated_block(self, in_type: nn.FieldType, channels: int, layers_num: int):
        ...
        for _ in range(layers_num):
            ...
            layers.append(nn.R3Conv(cur_type, full_type, kernel_size=self.kernel_size, padding=self.pad))
    ...
    def make_norm_block(self, in_type: nn.FieldType, channels: int, layers_num: int):
        ...
        for _ in range(layers_num):
            ...
            layers.append(nn.R3Conv(cur_type, feature_type, kernel_size=self.kernel_size, padding=self.pad))
