...
# Multiply by 100
agg_df["mean"] = agg_df["mean"] * 100
agg_df["std"] = agg_df["std"] * 100
...
# Filter rows: keep only activations that have data for 'eurosat'
if "eurosat_mean" in df.columns:
    # We want to keep rows where the activation (and model_type) has at least one entry with valid eurosat data.
    # Since df is pivoted by (model_type, activation, bn, aug), we can group by activation and check.
    
    # First, identify activations that have valid eurosat data
    valid_activations = set()
    for idx, row in df.iterrows():
        if pd.notna(row["eurosat_mean"]):
            act = _resolve_activation(row)
            valid_activations.add(act)
